  ...
  event.respondWith(
    (async () => {
        ...
        if (networkResponse && networkResponse.ok) {
          ...
          (async () => {
            try {
              ...
              console.warn("SW: failed to save network response", err?.message);
            }
          })();
      ...
      if (request.mode === "navigate") {
        try {
          const db = await openDB();
          const offline = await getAsset(
            db,
            makeKeyFromRequest("/offline.html")
          );
          const entry = await getAsset(db, "/event-logger/index.html");
